<?php
/* LuxCal display4 © 2009-2025 LuxSoft www.LuxSoft.eu*/$LDV='2024-12-20';
/*
...
$txtWrap = 0; //wrap long event texts to the next line (0: no, 1: yes)
$noBreak = 1; //remove line breaks in description and extra fields (0: no, 1: yes)
$showImg = 0; //show images when present in description and extra fields (0: no, 1: yes)
...
//EVENT FORMAT
$evtDate = "w. D/X"; //W: Monday, w: Mon, d: 8 (day), D: 08 (day), M: January, m: Jan, x: 5 (month), X: 05 (month)
...
$evtBody = 'venue: #v| cat: #c|, #1|, #2|, #3|, #a'; //event fields separated by "|" (#x is replaced as follows: #v: venue, #c: category, #a: link(s) to attachment(s), #1: description, #2: extra field 1, #3: extra field 2, #/: new line)
$tPrefix = ''; //prefix text for time field, if it exists. For instance '@
$LEVNT = "1.1"; //event line height (current font size multiplication factor)
$WDATE = "80px"; //date field width. "" (blank): no width
$MTITL = "0px"; //event title left margin
$MBODY = "6px"; //event body left margin
...
//DISPLAY COLORS
$BGENL = "#FFFFFF"; //general background color
$CBORD = "#1373D9"; //container border color
$CHEAD = "#1373D9"; //header text color
$CDATE = "#104070"; //calendar date text color
$BDATE = "#FFFFFF"; //calendar date background color
$CTIME = "#A02070"; //event time color
$BTIME = "#FFFFFF"; //calendar time background color
$CTITL = "#605060"; //event title color
$BTITL = "#FFFFFF"; //event title background color
$CEVNT = "#605060"; //event description color
$BEVNT = "#FFFFFF"; //event background color
$CLINK = "#208020"; //URL link color
...
function makeDate($date) { //format dates
	global $months, $months_m, $wkDays, $wkDays_l, $evtDate;

	$m = substr($date, 5, 2);
	$d = substr($date, 8, 2);
	$n = date("N", strtotime($date));
	$nDate = '';
	$dElms = ['W' => $wkDays[$n], 'w' => $wkDays_l[$n], 'M' => $months[$m - 1], 'm' => $months_m[$m - 1], 'd' => trim($d,'0'), 'D' => $d, 'x' => trim($m,'0'), 'X' => $m]; //possible date elements
	foreach(str_split($evtDate) as $c) {
...
function makeEvtHead($head,$evt,$date) { //make event head
	...
	if ($evtT and !$evt['ald']) { $evtT = $tPrefix.$evtT; }
...
function makeEvtBody($body,$evt) { //make event body
	global $xx, $noBreak, $showImg;

	if ($evt['att']) { //make attachement link(s)
		$attachments = [];
		...
		$attLinks = implode(', ',$attachments);
	}
	if ($evt['ven']) { $body = str_replace("#v",$evt['ven'],$body); }
	if ($evt['cnm']) { $body = str_replace("#c",$evt['cnm'],$body); }
	if ($evt['tx1']) { $body = str_replace("#1",$evt['tx1'],$body); }
	if ($evt['tx2']) { $body = str_replace("#2",$evt['tx2'],$body); }
	if ($evt['tx3']) { $body = str_replace("#3",$evt['tx3'],$body); }
	if (!empty($attLinks)) { $body = str_replace("#a",$attLinks, $body); }
	$bodyArr = explode('|',$body);
	$body = implode('', array_filter($bodyArr, function($v) { return strpos($v,'#') === false; }));
	if ($noBreak) { $body = str_replace('<br>','',$body); }
	if (!$showImg) { $body = preg_replace('~<img\s[^>]+>~','',$body); }
	return trim($body,' ,');
}
...
<meta name="application-name" content="LuxCal V<?= LCV.' Display3 V'.$LDV.' Calendar ID '.$calID?>">
...
div.noEvt {text-align:center; padding:30px 0; color:{$CEVNT}; font:{$FNOEV};}
div.event {line-height:{$LEVNT};".($txtWrap ? '' : ' white-space:nowrap;')."}
span.datim {font:{$FDATE}; color:{$CDATE}; background:{$BDATE}; display:inline-block; width:{$WDATE};}
span.eHead {font:{$FETIT}; color:{$CTITL}; background:{$BTITL}; margin-left:{$MTITL};}
span.eBody {color:{$CEVNT}; background:{$BEVNT}; margin-left:{$MBODY};}
span.eBody img {vertical-align:middle;}
...
if ($login and !$msg) { //display login/out button
	echo "<form class='login' action='".htmlspecialchars($_SERVER["REQUEST_URI"])."' method='post'>\n";
		echo $uID == 1 ? "<button type='submit' name='logX' value='w'>{$xx['log_in']}</button>" : "{$usr['name']}&ensp;<button type='submit' name='logX' value='o'>{$xx['log_out']}</button>\n";
	echo "</form>\n";
}
if ($calName) {
	echo "<div class=".(($evtWin and $usr['privs'] > 1) ? "'header point' onclick='newE(0);'" : "'header'").">{$calName}</div>\n";
}
...
<div class='floatC'><button type='submit' class='bold' name='logX' value='i'>{$xx['log_in']}</button>\n";
	...
	if ($evtList) {
		foreach ($evtList as $cDate => &$events) { //loop thru dates
			...
			$txtDate = makeDate($cDate);

			//show events for this date
			$now = date('Y-m-dH:i');
			foreach ($evtList[$cDate] as $evt) {
				if ($futEvts and $evt['eti'] and $cDate.$evt['eti'] < $now) { continue; } //future events only
				$chBox = $evt['cbx'] ? checkBox($evt,$cDate) : '';
				if ($evtWin) {
					$onClick = " onclick='".($evt['mayE'] ? 'editE' : 'showE')."({$evt['eid']},`{$cDate}`,`{$calID}`);'"; //view or post/edit 
				} else {
					$onClick = '';
				}
				$eHead = makeEvtHead($evtHead,$evt,$cDate); //make event head
				$eBody = makeEvtBody($evtBody,$evt); //make event body
				echo "<div class='event'>\n";
				echo "<span class='datim'{$onclick}>".makeDate($cDate)."</span>\n";
				echo "<span class='eHead'>{$chBox}<span{$onClick}>{$eHead}</span></span>\n";
				echo "<span class='eBody'>{$eBody}</span>\n";
				echo "</div>\n";
			}
			if (--$maxDays == 0) { break; }
		}
