<?php
/* LuxCal PDF Print Upcoming Events - Portrait class
...
*/
$pWidth = (strtolower($pSize) === 'letter') ? 215.9 : 210; //Letter or A4
$marginL = 20;
$marginR = 20;
...
function Header() { //page header
	global $font, $logo, $link, $title, $marginL, $marginR, $pWidth, $cHEAD, $bHEAD;
	...
	if (!empty($logo)) {
		$this->Image($logo,$marginL,6,14,0,'',$link);
	}
	$this->SetX(($pWidth - $w) / 2);
	$this->Cell($w,6,$title,0,0,'C',true,$link);
	$this->SetY(30);
	$this->SetLeftMargin($marginL + $this->body * 35); //set abscissa to left of column
...
function OpenTag($tag, $attr) { //opening tag
	...
	if ($tag=='BR')
		$this->Ln(5);
		$this->SetLeftMargin($marginL + 35); //set abscissa to left of column. important
}
...
function PrintMonth($monthTitle) { //month title
	global $font, $cMOYE, $bMOYE, $marginL;
...
function PrintDay($wDay) { //week day + day
	global $font, $cDATE, $bDATE, $marginL;
...
function PrintEvent($evtTime,$evtTitle,$evtBody,$hdStyle,$tColor='#505050',$tBgrnd='#FFFFFF') { //event details
	...
	$this->SetFillColor($r,$g,$b);
	$this->SetLeftMargin($marginL + 35);
	$this->SetX($marginL + 35);
