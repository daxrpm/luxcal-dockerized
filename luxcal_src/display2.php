<?php
/* LuxCal display2 © 2009-2025 LuxSoft www.LuxSoft.eu*/$LDV='2024-12-20';
/*
...
function showEvents($date) { //show events in calendar
	global $calID, $xx, $evtList, $evtHead, $evtBody, $futEvts, $evtWin, $rxULink, $rxIMGTags;
...
<meta name="application-name" content="LuxCal V<?= LCV.' Display2 V'.$LDV.' Calendar ID '.$calID?>">
...
<div class='floatC'><button type='submit' class='bold' name='logX' value='i'>{$xx['log_in']}</button>\n";
	...
	if ($evtList) {
		foreach ($evtList as $cDate => &$events) { //loop thru dates
			...
			echo "<tr>\n<td class='date'{$onclick} colspan='2'>".makeDate($cDate)."</td>\n</tr>\n"; //show date
