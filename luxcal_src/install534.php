<?php
/*
LuxCal Installation script - SQLite database version

!!!!!!! AFTER UPLOADING ALL LUXCAL FILES AND FOLDERS TO YOUR SERVER,   !!!!!!!
...
'mycal = My Web Calendar'. If multiple calendars are installed, then later - 
when starting a calendar - the calendar name is used to select a specific 
calendar.
</p>
...
if ($test or $install) {
	...
	if (version_compare(PHP_VERSION,'7.0.0') >= 0) {
		...
		$errMsg[] = "PHP version ".PHP_VERSION." too low. The calendar needs PHP version 7.0 or higher\n";
	}
	...
	$dbDerror = (!preg_match('~^[a-z\d_-]{1,20}$~i',$defCal)) ? ' class="hilite"' : '';
	$dbFerror = (!preg_match('~^(.{0,2}|\w+)/([^/]+/)*$~i',$dbDir)) ? ' class="hilite"' : '';
	$adNerror = !$adName ? ' class="hilite"' : '';
	$adEerror = (!filter_var($adMail,FILTER_VALIDATE_EMAIL)) ? ' class="hilite"' : '';
	...
	if (!$dbNerror) {
		...
		if (!$dbDerror and !array_key_exists($defCal,$allCals)) { //default calendar not existing
			$defCal = substr($newCals,0,strpos($newCals,' = ')); //set to first new calendar
		}
	} else {
...
if ($install and empty($errMsg)) {
	...
	foreach ($allCals as $name => $title) { //check / create calendar tables
		...
		//insert initial data in groups, users, cats, styles and settings tables
		initGroups();
